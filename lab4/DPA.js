class DPA {
    ...
    transition = (symbol) => {
        if (this.alphabet.includes(symbol)) {
            const state = this.transitions[this.currentState];
            if (state) {
                const result = state.rule(symbol, this.memory, this.tree);
    ...
    run = (string) => {
        ...
        if (!this.memory.length && this.finiteStates.includes(this.currentState)) {
            return ({result: true, tree: this.tree})
        }
        return false;
    };
}
